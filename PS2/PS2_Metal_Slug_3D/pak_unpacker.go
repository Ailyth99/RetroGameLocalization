...
func main() {
	...
	_, err = io.ReadFull(file, fullData)
	if err != nil {
		fmt.Printf("Error reading file: %v\n", err)
		return
	}

	reader := io.NewSectionReader(bytes.NewReader(fullData), 0, fileSize)
	
	err = extract(reader, outDir)
	
	if err != nil && err.Error() == "invalid format" {
		fmt.Println("Standard header not found. Switching to DEEP SCAN mode...")
		scanAndExtract(fullData, outDir)
	} else if err != nil {
...
//解压PKLZ
func DecompressPK02(src []byte, decompressedSize int) ([]byte, int, error) {
	...
	for len(dest) < decompressedSize {
		...
		if bitCount > 7 {
			if sp >= len(src) {
				if len(dest) >= decompressedSize { break }
				break // truncated
			}
			controlByte = src[sp]
			sp++
...
func scanAndExtract(data []byte, outDir string) {
	...
	i := 0
	for i < len(data)-16 {
		if data[i] == 0x50 && data[i+1] == 0x4B && data[i+2] == 0x00 && data[i+3] == 0x02 {
			decompSize := binary.LittleEndian.Uint32(data[i+4 : i+8])
			compDataStart := i + 16
			if compDataStart >= len(data) { break }
			
			decompressedData, bytesRead, err := DecompressPK02(data[compDataStart:], int(decompSize))
			
			if err == nil && len(decompressedData) == int(decompSize) {
				totalCompSize := 16 + bytesRead
				
				fileName := fmt.Sprintf("scan_%08x", i)
				fmt.Printf("  [SCAN] Found PKLZ at offset %08x (Comp: %d -> Decomp: %d)\n", i, totalCompSize, decompSize)

				pklzName := filepath.Join(outDir, fileName+".pklz")
				os.WriteFile(pklzName, data[i : i+totalCompSize], 0644)

				binName := filepath.Join(outDir, fileName+".bin")
				os.WriteFile(binName, decompressedData, 0644)
	...
	if foundCount == 0 {
		fmt.Println("  No PKLZ files found in deep scan.")
	} else {
		fmt.Printf("  Deep scan finished. Found %d files.\n", foundCount)
	}
}
...
func extract(reader *io.SectionReader, currentPath string) error {
	...
	if magic == "PK02" {
		...
		if int64(len(rawData)) <= 16 { return fmt.Errorf("file too small") }
		fmt.Printf("  -> Decompressing: %s (Decomp Size: %d)\n", filepath.Base(currentPath), decompSize)
		
		data, _, err := DecompressPK02(rawData[16:], decompSize)
		if err != nil { return err }
